...
I(x) = IntervalArithmetic.interval(prevfloat(Float64(x)), nextfloat(Float64(x))) 
# Converts a floating-point number to an ArbNumerics ArbReal pair. 
J(x) = setinterval(prevfloat(Float64(x)), nextfloat(Float64(x))) 
...
# Integer powers in ArbNumerics. 
function Q(X, power) # power must be a POSITIVE INTEGER 
    result = J(1) 
    for j = 1 : power 
        result = result * X 
    end 
...
Vol_sausage(N) = I(2) * (N - I(1)) * (I(4) / I(3) * I(π)) + I(1) / I(2) * I(π)*I(π) 
        G_Y(M) = I(4) * P(M, 4) + I(8) * P(M, 3) + I(8) * P(M, 2) + 4 * M + 1 
      Vol_Y(M) = I(32) * P(M, 4) 
...
function vol_t3h_Y(m, h) 
    ...
    # Components of vol(t_h_3(Z_m) + B^4). 
    c = 64 * √2 - 16 / 3 * h_(h)[1] 
...
function Check_n(n, details = false, fast = false) 
    ...
    if fast == true # Shortcuts for when fast == true 
        ...
        while max( G_Y(I(min_m)).hi, (G_Y(I(min_m)) - R̃(I(min_m), [h_max(I(min_m)) h_max(I(min_m)) h_max(I(min_m))])).hi ) < N.lo 
            min_m += 1 
        end 

        while G_t3h_Y(I(max_m), [h_max(I(max_m)) h_max(I(max_m)) h_max(I(max_m))]).hi ≤ N.lo 
            max_m += 1 
        end # Sometimes (for small n) max_m will be less than min_m — that's fine since we only use this code for n ≳ 500,000 anyway 
        max_m -= 1 
        ...
        for h3 = 0 : h_max(m) 
            for h2 = 0 : h3 
                for h1 = 0 : h2 
                    ...
                    loopCounter += 1 
                    if ( (range_of_packing[1] - I(0.5)).hi ≤ N.lo) & (N.hi ≤ (range_of_packing[2] + I(0.5)).lo ) == true # NOTE: These inequalities cannot both be true if p_k(t_h_3(Z_m)) is LESS dense than S_n 
...
#= Justification for the condition "if (range_of_packing[1].lo ≤ n) & (n ≤ range_of_packing[2].hi) == true": 
